...
class GNSSData(object):
    def __init__(self, time, position_lla, gnss_velocity, true_position_lla, true_velocity,true_attitude) -> None:
        self.gnss_time = time
        ...
        self.true_attitude_quat = true_attitude
    ...
    def read_gnss_data(data_path)->queue.Queue:
        ...
        true_attitude_df = pd.read_csv(os.path.join(data_path,'ref_att_quat.csv'))
        ...
        for i in tqdm(range(len(gnss_time_df)), desc="reading gnss data"):
            gnss_data = GNSSData(gnss_time_df.loc[i,'gps_time (sec)'],
                               ...
                               np.array([true_attitude_df.loc[i,'q1'],
                                         true_attitude_df.loc[i,'q2'],
                                         true_attitude_df.loc[i,'q3'],
                                         true_attitude_df.loc[i,'q0 ()']])
                                         )
