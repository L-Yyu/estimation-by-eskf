...
class ESKF(object):
    ...
    def Correct(self, curr_gnss_data):
        ...
        #self.Y = self.pos_ - (lla2ned(curr_gnss_data.position_lla, self.ref_pos_lla_)+np.random.normal(0,1,(3,1))) # 计算量测值
        self.Y = self.pos_ - lla2ned(curr_gnss_data.position_lla, self.ref_pos_lla_)
        
        e = self.Y - self.G @ self.X # 计算新息向量

        b = 0.99
        self.beta = self.beta/(self.beta+b)
        self.k += 1
        self.R = (1-self.beta) * self.R + self.beta * (e @ e.T-self.G @ self.P @ self.G.T) # 更新量测协方差矩阵
        #self.R = (1-1/self.k)*self.R + 1/self.k*(e @ e.T-self.G @ self.P @ self.G.T)
        #print(np.diag(np.diag(e @ e.T)))
        #print('x:{}\n beta:{}\n R:{}\n e:{}\n u:{}\n'.format(self.X, self.beta, self.R, e, e @ e.T-self.G @ self.P @ self.G.T))
        # os.system('pause')
...
if __name__ == "__main__":
    ...
    #imu_data_queue = IMUData.read_imu_data(imu_data_queue, './data/sim_52')
    #imu_data_queue = IMUData.read_imu_data(imu_data_queue, './data/sim_42')
    #imu_data_queue = IMUData.read_imu_data(imu_data_queue, './data/sim_43')

    gnss_data_queue = queue.Queue()
    gnss_data_queue = GNSSData.read_gnss_data(gnss_data_queue, data_path)
    #gnss_data_queue = GNSSData.read_gnss_data(gnss_data_queue, './data/sim_52')
    #gnss_data_queue = GNSSData.read_gnss_data(gnss_data_queue, './data/sim_42')
    #gnss_data_queue = GNSSData.read_gnss_data(gnss_data_queue, './data/sim_43')
