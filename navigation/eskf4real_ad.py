...
import os
import yaml
from data4real import IMUData, GNSSData
from tools import *
...
class ESKF(object):
    ...
    def Predict(self, last_imu_data:IMUData, curr_imu_data:IMUData):
        ...
        # 计算Cbb
        rotation_vector = curr_imu_data.imu_angle_increment # rad
        ...
        if False:
            print(self.curr_imu_data.imu_time)
            print(self.pos_)
            print(self.pos_lla_)
            print(self.velocity_)
            print(rm2euler(self.rotation_matrix_)*D2R)
    ...
    def Correct(self, curr_gnss_data:GNSSData):
        ...
        # 计算观测量和新息
        atn_pos = self.rotation_matrix_ @ self.antenna_lever_ + self.pos_
        self.Y = atn_pos - lla2ned(curr_gnss_data.position_lla, self.ref_pos_lla_)
        ...
        #print('k:{}\n beta:{}\n R:{}\n e:{}\n u:{}\n'.format(self.k, self.beta, self.R, e, e @ e.T-self.G @ self.P @ self.G.T))
